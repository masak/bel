...
use Language::Bel::Pair::AbstractValue qw(
    make_abstract_value
);
...
sub compile {
    ...
    my $dp = $bc->cdr($program);
    my $name = $bc->car($dp);

    my $ddp = $bc->cdr($dp);
    my $params = $bc->car($ddp);

    my $dddp = $bc->cdr($ddp);
    # XXX: this is wrong if an when below assumption is broken
    my $body = $bc->car($dddp);

    my $ddddp = $bc->cdr($dddp);
    die "This case does happen (with 'enq'), but punting on it for now"
        unless is_nil($ddddp);

    my $args = make_abstract_value({ type => "argument" });
    my $env = SYMBOL_NIL;
    ...
    $bc->applyclo($params, $args, $env, $body);
    ...
    while (@{$bc->{s}} || @{$bc->{p}}) {
        ...
        #                  (snoc p (list s r)))
        #              g)))
        my $b = $bc->binding(SYMBOL_LOCK);
        if (!$b || is_nil($bc->cdr($b))) {
            if (@{$bc->{s}}) {
                my $s = $bc->{s};
                my $r = $bc->{r};
                push @{$bc->{p}}, [$s, $r];
